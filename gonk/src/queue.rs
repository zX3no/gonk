...
impl Widget for Queue {
    ...
    fn draw(&mut self, f: &mut Frame, _: Rect, mouse_event: Option<MouseEvent>) {
        let player = unsafe { self.player.as_mut().unwrap() };
        ...
        let row_bounds = if log::last_message().is_some() && player.songs.is_empty() {
            ...
            let (songs, player_index, ui_index) =
                (&player.songs, player.songs.index(), self.ui.index());
                        ...
                        if ui_index != player_index {
                            if let Some(song) = songs.get(ui_index) {
                                let row = Row::new(vec![
                                    ...
                                    Cell::from(song.title.as_str())
                                        .style(Style::default().bg(TITLE)),
                                    Cell::from(song.album.as_str())
                                        .style(Style::default().bg(ALBUM)),
                                    Cell::from(song.artist.as_str())
                                        .style(Style::default().bg(ARTIST)),
                                ])
