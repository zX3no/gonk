...
use crate::cpal::traits::{DeviceTrait, HostTrait, StreamTrait};
use crate::cpal::{
...
impl Host {
    pub fn new() -> Result<Self, crate::cpal::HostUnavailable> {
        Ok(Host)
    }
}
...
impl Device {
    fn build_stream_inner(
        ...
        let can_pause = match set_hw_params_from_format(&handle, conf, sample_format) {
            Ok(can_pause) => can_pause,
            Err(err) => return Err(BuildStreamError::BackendSpecific { err }),
        };

        let period_len = match set_sw_params_from_format(&handle, conf, stream_type) {
            Ok(period_len) => period_len,
            Err(err) => return Err(BuildStreamError::BackendSpecific { err }),
        };
        ...
        let mut formats: Vec<_> = {
            match self.supported_configs(stream_t) {
                ...
                Err(SupportedStreamConfigsError::BackendSpecific { err }) => {
                    return Err(DefaultStreamConfigError::BackendSpecific { err });
                }
...
fn process_input(
    ...
    let timestamp = crate::cpal::InputStreamTimestamp { callback, capture };
    let info = crate::cpal::InputCallbackInfo { timestamp };
...
fn process_output(
...
) -> Result<(), BackendSpecificError> {
    {
        ...
        let timestamp = crate::cpal::OutputStreamTimestamp { callback, playback };
        let info = crate::cpal::OutputCallbackInfo { timestamp };
...
fn stream_timestamp(
    status: &alsa::pcm::Status,
    creation_instant: Option<std::time::Instant>,
) -> Result<crate::cpal::StreamInstant, BackendSpecificError> {
    match creation_instant {
        None => {
            ...
            Ok(crate::cpal::StreamInstant::from_nanos(nanos))
        }
        Some(creation) => {
            let now = std::time::Instant::now();
            let duration = now.duration_since(creation);
            let instant = crate::cpal::StreamInstant::from_nanos_i128(duration.as_nanos() as i128)
...
fn frames_to_duration(frames: usize, rate: crate::cpal::SampleRate) -> std::time::Duration {
