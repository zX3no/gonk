...
use crate::cpal::{traits::DeviceTrait, BuildStreamError, StreamError};
...
impl Device {
    pub fn name(&self) -> Result<String, DeviceNameError> {
        unsafe {
            ...
            (*self.device).OpenPropertyStore(STGM_READ, &mut property_store);
            ...
            if let Err(err) = check_result((*property_store).GetValue(
                ...
                return Err(DeviceNameError::BackendSpecific { err });
            }
    ...
    fn supported_formats(&self) -> Result<SupportedInputConfigs, SupportedStreamConfigsError> {
        ...
        let lock = match self.ensure_future_audio_client() {
            ...
            Err(e) => {
                ...
                return Err(SupportedStreamConfigsError::BackendSpecific { err });
            }
        };
        let client = lock.unwrap().0;
...
impl Devices {
    pub fn new() -> Result<Self, BackendSpecificError> {
        unsafe {
