...
pub struct StreamInner {
    ...
    pub config: crate::cpal::StreamConfig,
...
fn stream_error_from_hresult(hresult: winnt::HRESULT) -> Result<(), StreamError> {
    ...
    if let Err(err) = check_result(hresult) {
        ...
        let err = BackendSpecificError { description };
        return Err(StreamError::BackendSpecific { err });
    }
    Ok(())
}
...
fn stream_instant(stream: &StreamInner) -> Result<crate::cpal::StreamInstant, StreamError> {
    ...
    let instant = crate::cpal::StreamInstant::from_nanos_i128(qpc_nanos)
...
) -> Result<crate::cpal::InputStreamTimestamp, StreamError> {
    // The `qpc_position` is in 100 nanosecond units. Convert it to nanoseconds.
    let qpc_nanos = buffer_qpc_position as i128 * 100;
    let capture = crate::cpal::StreamInstant::from_nanos_i128(qpc_nanos)
        .expect("performance counter out of range of `StreamInstant` representation");
...
fn output_timestamp(
    ...
    sample_rate: crate::cpal::SampleRate,
) -> Result<crate::cpal::OutputStreamTimestamp, StreamError> {
    let callback = stream_instant(stream)?;
    let buffer_duration = frames_to_duration(frames_available, sample_rate);
    let playback = callback
        .add(buffer_duration)
        .expect("`playback` occurs beyond representation supported by `StreamInstant`");
    Ok(crate::cpal::OutputStreamTimestamp { callback, playback })
}
