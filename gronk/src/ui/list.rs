...
impl<'a> List<'a> {
    ...
    fn get_items_bounds(
        ...
        let selection = selected.unwrap_or(0).min(self.items.len() - 1);
...
impl<'a> StatefulWidget for List<'a> {
    ...
    fn render(mut self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        ...
        //
        let (start, end) =
            self.get_items_bounds(state.selected, state.offset, list_area.height as usize);
