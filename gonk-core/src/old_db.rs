...
pub fn read() -> Result<Vec<Song>, Box<dyn Error + Send + Sync>> {
    ...
    let song_count = bytes.len() / old_db::SONG_LEN;
    //Keep track of the database size.
    unsafe { LEN = song_count };

    if bytes.len() % old_db::SONG_LEN != 0 {
    ...
    let songs: Vec<Song> = (0..song_count)
        .into_par_iter()
        .flat_map(|i| {
            let pos = i * old_db::SONG_LEN;
            let bytes = &bytes.get(pos..pos + old_db::SONG_LEN).ok_or("Invalid song")?;
            old_db::bytes_to_song(bytes)
        })
        .collect();
